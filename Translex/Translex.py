...
def translate():
    ...
    if language_1 == '':
        messagebox.showerror('Translex', 'please fill the box')
    else:
        t2.delete(1.0, 'end')
...
img = ImageTk.PhotoImage(Image.open('Translex.png'))
label = Label(image=img)
label.place(x=0, y=0)

a = tk.StringVar()
auto_detect = ttk.Combobox(root, width=20, textvariable=a, state='readonly', font=('arial', 10, 'bold'), )
...
l = tk.StringVar()
choose_langauge = ttk.Combobox(root, width=20, textvariable=l, state='readonly', font=('arial', 10, 'bold'))
